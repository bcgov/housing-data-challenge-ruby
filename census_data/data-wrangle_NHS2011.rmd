...
## Socio-demographic statistics: by Dissemination Area, Census Tract, etc
...
read source file and filter so only Dissemination Area (DA) & Census Division (CD) / Census Sub-Division (CSD) stats remain
...
NHS2011data <- read_csv("R3_SCP_Can_PR_CD_CSD_DA_AllGNRs_withSUPP_BC.csv",
                        col_types = cols(.default = "d",
                                         Geography = "c"))

write_feather(NHS2011data, "NHS2011data.feather")
#NHS2011data <- read_feather("NHS2011data.feather")
...
write_feather(NHS2011data_DA, "NHS2011data_DA.feather")
#NHS2011data_DA <- read_feather("NHS2011data_DA.feather")


# filter to just municipalities

NHS2011data_CDCSD <- NHS2011data %>%
  filter(!str_detect(Geography, '59'))

write_feather(NHS2011data_CDCSD, "NHS2011data_CDCSD.feather")
#NHS2011data_CDCSD <- read_feather("NHS2011data_CDCSD.feather")
...
* see Statistics Canada, [National Household Survey Dictionary](http://www12.statcan.gc.ca/nhs-enm/2011/ref/dict/index-eng.cfm)
...
pop_mobility_DA <- NHS2011data_DA %>%
  select(1:2,1718:1771)

head(pop_mobility_DA)

write_feather(pop_mobility_DA, "pop_mobility_DA.feather")

pop_mobility_CDCSD <- NHS2011data_CDCSD %>%
  select(1:2,1718:1771)

head(pop_mobility_CDCSD)

write_feather(pop_mobility_CDCSD, "pop_mobility_CDCSD.feather")

# need to prune further!

# M1 - mobility from 2010 (one year previous), total population

pop_mobility_M1t_CDCSD <- NHS2011data_CDCSD %>%
  select(1:2,1718:1726)

View(pop_mobility_M1t_CDCSD)
ls.str(pop_mobility_M1t_CDCSD)

pop_mobility_M1t_CDCSD <- pop_mobility_M1t_CDCSD %>%
  mutate(totalpop = (`Non-movers` + `Non-migrants` + `Interprovincial migrants` + `Intraprovincial migrants` + `External migrants`)) %>%
  mutate(check_totalpop = totalpop - `Total - Mobility status 1 year ago`) 

# function to calculate share of total

fun_pct_totalpop <- function(mobgrp) {
  pct_mobgrp <- mobgrp / pop_mobility_M1t_CDCSD$`Total - Mobility status 1 year ago`
}

#

pop_mobility_M1t_CDCSD <- pop_mobility_M1t_CDCSD %>%
  mutate(Nonmover_pct = fun_pct_totalpop(`Non-movers`))
